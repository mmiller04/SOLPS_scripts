# -*- coding: utf-8 -*-
"""
Routines to output results from SOLPS based on
R. Reksoatmodjo's VesselPlotterNew.py
"""
import os
import numpy as np
import xarray as xr
    ...
    ** Setup **
    ...
    def __init__(self, Shot, Attempts, Parameters=None, **kwargs):
        ...
        self.DefaultSettings = {'TimeRange' : [1.10,1.30],  
                     ...
                     'GRID': False,
                     'AX' : None,
                     'BASEDRT': '/nobackup1/millerma/solps-iter/runs',
                     'TOPDRT' : '',
                     'ROOTSHOT' : ''} #1160718
    ...
    def _LoadSOLPSData(self, AddNew=None):
        ...
        if 'd3d' in Shot or DEV=='d3d':
            ...
            if ROOTSHOT == '':
                ...
                GFILE = glob.glob('{}/exp_data/gfiles/g{}*'.format(BASEDRT,Shot))
                ...
                if EXP:
                    ...
                    ExpFile = '{}/exp_data/lyman_data_{}.pkl'.format(BASEDRT,Shot)
                    ...
                    ExpData = {}
                    ExpData['rhop'] = PklData[0]
                    ExpData['rmid'] = PklData[2]
                    ExpData['ne'] = PklData[7]
                    ExpData['nerr'] = PklData[8]
                    ExpData['te'] = PklData[9]
                    ExpData['terr'] = PklData[10]

                # may need to change where these are defined when exp is fixed
            ...
            if EXP:
                ...
                Rmid = ExpData['rmid']
                RmidAvg = np.mean(Rmid)
    
                try:
                    #Psin = ExpData['psin'][:,ti:tf]
                    #PsinAvg = np.mean(Psin, axis=1)
                    PsinAvg = ExpData['rhop']**2 # psin = rhop^2
                except:
                ...
                self.ExpDict['NemidAvg'] = ExpData['ne']
                self.ExpDict['ErrNe'] = ErrNe
                self.ExpDict['TemidAvg'] = ExpData['te']
        ...
        self.KW['SEP'] = SEP
        self.Attempts = Attempts
        self.VVFILE = np.loadtxt('{}/baserun/vvfile.ogr'.format(BASEDRT))
        self.GF = GF
        self.Xx = Xx
        self.Yy = Yy
        self.Xpoint = Xpoint

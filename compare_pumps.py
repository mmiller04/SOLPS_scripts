...
b2path9 = '{}/{}/pump{}/attempt{}'.format(SOLPSWORK, shot_on, 9, attempt)
...
ax.plot(R0_LFS,np.log(nn0),'.')
ax.plot(R4_LFS,np.log(nn4),'.')
ax.plot(R9_LFS,np.log(nn9),'.')
...
ax.plot(R0_LFS,np.log(sna0_OMP),'.')
ax.plot(R4_LFS,np.log(sna4_OMP),'.')
ax.plot(R9_LFS,np.log(sna9_OMP),'.')
ax.legend(['pump0', 'pump4', 'pump9', '1070614013 (on)', '1070614016 (off)'])
ax.axvline(R_sep_SOLPS,linestyle='--',color='gray')
#ax[0,1].plot(SOLPS['te']['X'], SOLPS['te']['Y'])
#ax[0,1].set_ylabel('$S_{ion}$')
ax.set_ylabel('$log(S_{D^{+}}) (m^{-3}s^{-1})$')
ax.set_xlabel('$R (m)$')
...
ax.plot(R0_LFS,np.log(flux0),'.')
ax.plot(R4_LFS,np.log(flux4),'.')
ax.plot(R9_LFS,np.log(flux9),'.')
